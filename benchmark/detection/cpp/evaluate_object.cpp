...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,const vector< vector<tGroundtruth> > &groundtruth,const vector< vector<tDetection> > &detections, bool compute_aos, vector<double> &precision, vector<double> &aos, DIFFICULTY difficulty, int top_stats) {
  ...
  printf("enter eval_class. --zhihengw\n");
...
bool eval(string result_sha,Mail* mail,int top_stats){
  ...
  if(eval_car){
    ...
    for(int i=0; i < (int)precision[1].size(); ++i){
      printf("%f\n", precision[1][i]);
    }
  ...
  if(eval_cyclist){
    ...
    printf("eval finish and prepare to save result. --zhihengw\n");
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  // run evaluation
  int top_stats = 0;
  if (argc==3) top_stats = atoi(argv[2]);
  bool success = eval(result_sha,mail,top_stats);
  //if (argc==4) mail->finalize(success,"object",result_sha,argv[2]);
  //else         mail->finalize(success,"object",result_sha);
